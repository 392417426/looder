...
## 2、怎样将页面组件化?
* 把一个页面中各个栏目分装成一个组件，组件文件里面可以同时有html，css，js，
然后页面上可以通过标签来调用。
...
## 5、gulp是什么？
* gulp是一个自动化构建项目流程的工具。
在配置文件中添加一个名称为less的任务，然后设置要处理的文件来源以及处理后的文件目录，最后设置监听器监听这个任务
#### 插件:
1. js：gulp-jshint
...
## 14、应用缓存是什么？
* 当应用不联网时，仍然可以访问网站的内容。可以减少服务器的压力，浏览器只需要从服务器上下载更新过的
资源，提高加载速度。新建一个minifest文件，通过在头部设置manifest文件路径，在manifest上设置要缓存的文件。
...
## 为什么离开现在的公司？
* 因为我公司以前的技术都在广州，后面我们的主管走了，
    然后北京那边有个产品经理在那边组建了个技术部，慢慢地取缔广州的技术部了，广州这边的人离职了，也不招了。
    我觉得公司不重视广州技术部，工资待遇也提不了

## 你的职业规划是怎么样的？
* 在五年内，我打算都是做前端开发的，能够在技术方面独当一面，扩大技术面。
...
自我介绍
我叫李炯鹏，毕业于广东轻工业职业技术学院。
我从14年毕业到现在，有3年前端经验了，大大小小的项目也做过5、6个了，有跟别人合作的，也有独立完成的项目。
我掌握的技术有h5、css3、javascript、jquery、boostrap、less、gulp等。平常工作有空的时候就看一下前端的新技术，
例如webpack、vuejs等。在未来5年内，我还是打算继续做前端开发的。我觉得前端开发的发展前景是很好的
