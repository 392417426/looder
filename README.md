...
## js数据类型：
* null、undefinded、number、boolean、string、object、symbol、bigint
...
## this的理解
this在函数内部，谁调用了这个函数，this指向谁。
在全局作用域下，this指向window，在es5严格模式下，this指向undefined
...
absolute相对于 static 定位以外的第一个父元素进行定位
relative相对于元素本身位置定位

## BFC的理解
它是一块独立的渲染区域，它会隔绝内部元素与外部元素的联系，不同的BFC区域，进行渲染互不干扰，常用于解决高度坍塌，外边距重叠等问题
...
addEventListener 默认为false 第三个参数为true，事件会在捕获阶段执行，为false，事件会在冒泡阶段执行
...
* 内存溢出 一种程序运行出现的错误，当程序运行需要的内存超过剩余内存时就会抛出内存溢出的错误

## var、let的区别
* var存在变量的提升，let可以认为它没有变量的提升，存在暂时性死区，只在当前的作用域生效
* var可以重复声明变量，let不能重复声明变量

## ts中协变与逆变
协变：小类型可以赋值给大类型，大类型不能赋值给小类型
逆变：在函数里面，参数传参

## ts中type和interface
相同之处：都可以定义对象和函数，都可以实现类的继承
不同之处：实现类的继承方式不一样，type可以定义基本类型和联合类型，infertace可以重复声明合并-

## 箭头函数和普通函数的区别
箭头函数的this指向上一级作用域下的this，不能通过call，bind改变this的指向
箭头函数不能作为构造函数
箭头函数没有arguments对象


## 页签通讯方式
1.BroastcastChannel 广播频道(同源)
2.shared worker和service worker(同源)
3.locastorage，onstorage监听(同源)
4.window.postmessage（跨域）
5.websocket

## react refs的作用是什么？
refs就是通过forwardRef转发ref到子组件的某个dom元素上面

## react for循环的key的作用是什么？
元素的唯一标识，加上key可以复用元素，减少创建元素和渲染元素的性能上的消耗

## 概述一下react事件处理逻辑
在应用第一次渲染时，react会在根节点注册原生事件。原生事件触发时，react会模拟捕获、目标和冒泡阶段派发合成事件。

## 描述一下什么是redux
redux是全局状态管理的框架，把状态集中管理，使状态变化是可以预测的，三大原则：单一数据源，state是只读的，使用纯函数来执行修改

## 当调用setState时，发生了什么事情？
把传进去的参数对象和当前的状态合并，这样会启动一个和解过程，创建新的react dom树，然后与旧的react dom树进行对比。进行最小化的渲染

## 什么时候用函数式组件，什么时候用类组件？
对组件状态有要求的，就使用类组件，对组件状态无要求的，使用函数式组件

## 函数式组件和类组件的区别？
类组件是面向对象编程，它有内部状态管理和生命周期，并具有继承能力
函数组件属于函数式编程，不存在内部状态管理，没有继承能力
类组件可以获取实例化的this，函数式组件不行

## 为什么react中很多个 setState 为什么是执行完再 render
合并更新可以减少渲染的次数，提高渲染性能

## 描述一下hoc高阶组件和render props
高阶组件是复用组件逻辑的一种高级技巧，以组件作为参数返回一个新组件的函数；
render props是把函数作为props传入到其它组件，其它组件调用该函数进行渲染。
